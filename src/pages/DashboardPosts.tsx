...
const DashboardPosts = () => {
  ...
  const sortedPosts = useMemo(() => {
    ...
    if (sortMode === "tags") {
      // Apply Schwartzian transform to avoid O(N log N) array merging and joining
      const mapped = filteredPosts.map((post) => {
        const tags = [
          ...(post.projectId ? projectMap.get(post.projectId)?.tags || [] : []),
          ...(Array.isArray(post.tags) ? post.tags : []),
        ];
        return { post, tagString: tags.join(",") };
      });
      mapped.sort((a, b) => comparePtBrVariant(a.tagString, b.tagString));
      return mapped.map((w) => w.post);
    }
    ...
    next.sort((a, b) => {
      if (sortMode === "alpha") {
        return comparePtBrVariant(a.title, b.title);
      }
      if (sortMode === "status") {
        return comparePtBrVariant(a.status, b.status);
      }
      if (sortMode === "views") {
